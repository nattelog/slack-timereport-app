...
function getThisWeek(rows) {
  var ref = new Date();
  var lastMonday, nextMonday, daysToLastMonday, daysToNextMonday;
  var result = [];
  ...
  if (ref.getDay() === 0) {
    ...
    daysToLastMonday = ref.getDay() - 1;
    daysToNextMonday = 7 - daysToLastMonday;
  }

  lastMonday = new Date(ref.getFullYear(), ref.getMonth(), ref.getDate() - daysToLastMonday);
  nextMonday = new Date(ref.getFullYear(), ref.getMonth(), ref.getDate() + daysToNextMonday);

  rows.forEach(function(row) {
    var date = convertToDate(row.date, row.start);

    if (date >= lastMonday && date < nextMonday) {
      result.push(row);
    }
  });
